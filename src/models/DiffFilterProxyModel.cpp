...
bool DiffFilterProxyModel::filterAcceptsRow(int source_row, const QModelIndex &source_parent) const
{
    ...
    if (cols <= DiffEntryColumns::DETAILS) return true;
    ...
    const QModelIndex circuitIdx = sourceModel()->index(source_row, DiffEntryColumns::TYPE, source_parent);
    ...
    const int circuitId = circuitIdx.isValid() ? sourceModel()->data(circuitIdx, Qt::UserRole).toInt() : -1;
