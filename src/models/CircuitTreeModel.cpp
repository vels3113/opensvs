...
CircuitTreeModel::Node *CircuitTreeModel::buildNode(NetgenJsonParser::Report::Circuit *circuit, Node *parent)
{
...
void CircuitTreeModel::setCircuits(QVector<NetgenJsonParser::Report::Circuit> *circuits)
{
    beginResetModel();
    clear();
    circuits_ = circuits;
...
QModelIndex CircuitTreeModel::index(int row, int column, const QModelIndex &parentIdx) const
{
    if (column != 0) return {};
...
int CircuitTreeModel::columnCount(const QModelIndex &) const
{
    return 1;
}
...
NetgenJsonParser::Report::Circuit *CircuitTreeModel::circuitForIndex(const QModelIndex &idx) const
{
    Node *node = nodeFromIndex(idx);
    return node ? node->circuit : nullptr;
}

CircuitTreeModel::Node *CircuitTreeModel::nodeFromIndex(const QModelIndex &idx) const
{
