...
void NetgenJsonParserTests::parses_tut2_fixture()
{
    ...
    QVERIFY(sub.diffs[0].details.contains(QStringLiteral("The following pins are connected only in circuit A: inverter:Gnd (2)")));
    QCOMPARE(sub.diffs[1].name, QStringLiteral("Vdd"));
    QVERIFY(sub.diffs[1].details.contains(QStringLiteral("The following pins are connected only in circuit A: inverter:Vdd (2)")));
...
void NetgenJsonParserTests::parses_tut3_fixture()
{
    ...
    QVERIFY(sub.diffs[0].details.contains(QStringLiteral("The following pins are connected only in circuit A: pfet:bulk (1) | The following pins are connected only in circuit B: pfet:bulk (2), pfet:drain|source (2), pfet:gate (1)")));
